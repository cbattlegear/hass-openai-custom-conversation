...
- Optionally set **Extra model arguments (JSON)** to pass additional fields in the request body sent to the model, for example `{"frequency_penalty": 0.5, "seed": 42}`. Values are sent via the OpenAI client's `extra_body`, so provider-specific parameters work too, including nested ones such as `{"chat_template_kwargs": {"enable_thinking": false}}` (used by some vLLM servers). These override the other settings (such as temperature).
